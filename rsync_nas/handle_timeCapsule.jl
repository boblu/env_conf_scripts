...
#================================
TIMECAPSULE_IP="10.0.1.1"        # e.g. "192.168.1.100"
TIMECAPSULE_VOLUME="/Data"       # also try "/Data"
TIMECAPSULE_PASSWORD="henrylu"   # prefix special characters, e.g. \!
MOUNT_POINT=/mnt/JLData	         # no need to create the directory
#================================

IS_MOUNTED=`/bin/mount 2> /dev/null | grep "$MOUNT_POINT" | cut -d' ' -f3`
...
if [[ "$IS_MOUNTED" ]] ;then
	...
	if [[ "$CHECK_TIMECAPSULE" =~ "Apple" ]] ;then
		mkdir $MOUNT_POINT && /sbin/mount.cifs $TIMECAPSULE_PATH $MOUNT_POINT -o pass=$TIMECAPSULE_PASSWORD,sec=ntlm
	fi
fi
